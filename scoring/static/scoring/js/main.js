...
forEachElement('.donation-modal', function(donationModal){
    var modal = new bootstrap.Modal(donationModal);
    ...
    var shouldShowModal = function() {
        ...
        if ( localStorage.getItem('submitted-donation-modal-timestamp') ) {
            // Browser has already submitted the donation form.
            return false;
        }
        if ( localStorage.getItem('skipped-donation-modal-timestamp') ) {
            // Respect skipped modals for 3 days (fundraiser only runs for 7 days).
            var skippedTimestamp = localStorage.getItem('skipped-donation-modal-timestamp');
            var nowTimestamp = (new Date()).getTime() / 1000;
            var coolingOffPeriod = 60 * 60 * 24 * 3;
    ...
    donationModal.addEventListener('hide.bs.modal', function(e) {
        if ( ! localStorage.getItem('submitted-donation-modal-timestamp') ) {
            var timestamp = (new Date()).getTime() / 1000;
            localStorage.setItem('skipped-donation-modal-timestamp', timestamp);
        }
    });
    ...
    donationModal.addEventListener('click', function(e) {
        if ( e.target.closest('.btn') ) {
            if ( window.localStorage ) {
                var timestamp = (new Date()).getTime() / 1000;
                localStorage.setItem('submitted-donation-modal-timestamp', timestamp);
            }
            modal.hide();
        }
    });
    ...
    if ( shouldShowModal() ) {
        localStorage.removeItem('skipped-donation-modal-timestamp');
        modal.show();
    }
})
